...
public class QuantrihethongController implements Initializable {
	...
	private void register(ActionEvent event) throws SQLException {
		...
		test=true;
		 if(ID.equals("")||ID.equals(null)||password.equals("")||password.equals(null)||name.equals("")||name.equals(null)||position.equals("")||position.equals(null)) {
			test=false;
		...
		jtfpassword.setDisable(false);
		jtfposition.setDisable(false);
	 ...
	 @FXML
	  ...
	  @FXML
	    public void test(ActionEvent e) {
		  ...
		  ResultSet rs;
		try {
			rs = con.createStatement().executeQuery("select name,position from quanlynhansu where BINARY ID LIKE '%" + jtfID.getText() + "%'");
			  while (rs.next()) {
				 name=rs.getString(1);
        ...
        if(name==null) {
        	...
        	try {
    			rs = con.createStatement().executeQuery("select * from dangnhap where BINARY ID LIKE '%" + jtfID.getText() + "%'");
    			  if(rs.next()) {
    				  JOptionPane.showMessageDialog(null,"Nhân viên này đã có tài khoản!" );
    				  jtfID.setText("");
    			  }
    			  else {
    				  JOptionPane.showMessageDialog(null, "Mã nhân viên tồn tại và có thể tạo tài khoản!");
    					 jtfname.setText(name);
    			         jtfposition.setText(position);
    			         jtfpassword.setText(jtfID.getText());      
    				     jtfpassword.setDisable(true);
    				     jtfname.setDisable(true);
    				     jtfposition.setDisable(true);
    			  }
    		 } catch (SQLException ex) {
    	            System.err.println("Error"+ex);
    	        }
